   ...
   elseif ($opcionMenu==1 || $opcionMenu==2) {  //prestamo y devoluci�n
      ...
      if ($opcionMenu==1) { //Pr�stamo
        if($flds_PRES_ESTADO==5)
         ...
         } else if($flds_PRES_ESTADO==7){
            $encabezado.="rechazado"; 
         }
...
<?    $query="select PARAM_CODI, PARAM_VALOR from SGD_PARAMETRO where PARAM_NOMB='PRESTAMO_ESTADO' and PARAM_CODI in (2,5,6,7) ORDER BY PARAM_CODI"; 
   $rs = $db->conn->query($query);
	while($rs && !$rs->EOF) {
