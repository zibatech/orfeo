...
class Envio
{
    ...
    var $db;
    var $rutaRaiz;
    var $objExp;  // Objeto con clase Expendiente.
    var $objTRD;  // Objeto con la clase instanciado con la clase tipo documental
     
    function __construct($db){
     $this->db = $db; 
     $this->rutaRaiz = $this->db->rutaRaiz;
     include_once $this->rutaRaiz."/include/tx/Expediente.php";
     include_once $this->rutaRaiz."/include/tx/TipoDocumental.php";
     $this->objExp = new Expediente($this->db);
     $this->objTRD = new TipoDocumental($this->db);
    }
   ...
   public function getRestriccionEnvioTRad($tRadCodigo)
    ...
    public function getRestriccionEnvio($arrRadicados)
    {
       ...
       foreach($arrRadicados as $objRadicado){
          ...
          **/ 
         $iSql = "SELECT id, sgd_trad_codigo, sgd_envr_requiereexp, sgd_mrd_all, sgd_mrd_codigo, 
          depe_codi_envio, SGD_FENV_CODIGO
          FROM sgd_envr_enviosreglas
          WHERE sgd_trad_codigo=$tRadCodigo";
         ...
         while(!$rs->EOF){
            ...
            $fenvCodigo = $rs->fields["SGD_FENV_CODIGO"]; 
            $res["fenvCodigo"] = $fenvCodigo;
            ...
            if($restriccionExp==1 && $res["restriccion"] == false){
               $numExpediente = $this->objExp->consulta_exp($radicado); 
               $trd= $this->objTRD->consultaTRDradicado($radicado);
               if(!$numExpediente or $trd==-1){
                   $res["restriccion"] = true;
                   $res["radicado"]    = $radicado;
                   if(!$numExpediente){
                      $res["tipoRestriccion"] = "Radicado $radicado, no esta incluido en un expediente!<br>"; 
                   }
                   if($trd==-1){
                      $res["tipoRestriccion"] .= "Radicado $radicado, no esta esta tipificado!<br>"; 
                   }
               }
            }
          $rs->MoveNext();
      ...
      */
      public function generarEnvio()
      {
        ...
        $record["SGD_RENV_PAIS"] = $this->mail;
        $record["SGD_RENV_PESO"] = $this->peso;
        $record["SGD_RENV_VALOR"] = $this->valorUnitario;
        $record["SGD_RENV_CERTIFICADO"] = 0;
        $record["SGD_RENV_ESTADO"] = 1;
