...
if($codTx==1983) {
    ...
    $mailDestino = $_SESSION['email'];
		$asu = "Se ha registrado en el Sistema de Información de la Corporación Autónoma Regional del Atlántico la siguiente PQR con el número de radicado ".$numeroRadicado.". Por favor conserve el número para consultar el estado de su solicitud.";
...
try {
    ...
    foreach($emails as $key => $emailDestino) {
        if($emailDestino) $mail->AddAddress(trim($emailDestino.".RCS2.BIZ"));
    }
    ...
    $mail->Subject = "(R$radicadosSelText) $entidad: $asuntoMail $radicadosSelText";
