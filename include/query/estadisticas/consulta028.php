	...
	{	
		...
		{ 
			...
			$queryE = "	SELECT
						...
						WHERE ".$db->conn->SQLDate('Y/m/d', 'r.radi_fech_radi')." BETWEEN '$fecha_ini' AND '$fecha_fin' AND  r.radi_nume_radi not in (select anexos.anex_radi_nume from anexos) 
		...
		$queryEDetalle = "SELECT
						...
						WHERE
							...
							$filtroUsuario
							AND TO_CHAR(r.radi_fech_radi,'YYYY/MM/DD') BETWEEN '$fecha_ini' AND '$fecha_fin' $whereTipoRadicado $whereDependencia AND  r.radi_nume_radi not in (select anexos.anex_radi_nume from anexos)  ";
...
if($genDetalle==1){
	...
	$titulos=array("#","1#USUARIO","2#Radicados Sin Anexo","1#VER");
