...
//	echo "<pre>$queryEDetalle</pre>";
	...
	{	
		...
		if($tipoDocumentos!='999999')
		{	
				$queryE = "SELECT  b.depe_codi||' '||b.DEPE_NOMB 	as DEPENDENCIA,
				count($radi_nume_radi) 			as RADICADOS
				,b.DEPE_CODI HID_DEPE_USUA
				FROM RADICADO r, DEPENDENCIA b 
			WHERE
				r.RADI_DEPE_ACTU=b.DEPE_CODI AND  
				$condicionE
				$whereTipoRadicado 
				GROUP BY b.depe_codi||' '||b.DEPE_NOMB, b.DEPE_CODI
			ORDER BY $orno $ascdesc"; 
		}else{
			$queryE = "SELECT b.depe_codi||' '||b.depe_nomb 		as USUARIO,
						t.SGD_TPR_DESCRIP 		as TIPO_DOCUMENTO,
						count($radi_nume_radi) 	as RADICADOS,
            MIN(b.depe_codi) as HID_DEPE_USUA ,
						MIN(SGD_TPR_CODIGO) 	as HID_TPR_CODIGO			
			FROM RADICADO r 
			INNER JOIN DEPENDENCIA b ON r.RADI_DEPE_ACTU=b.DEPE_CODI  
			LEFT OUTER JOIN SGD_TPR_TPDCUMENTO t ON r.tdoc_codi=t.SGD_TPR_CODIGO
			WHERE $condicionE $whereTipoRadicado  
			GROUP BY b.depe_codi||' '||b.depe_nomb, t.SGD_TPR_DESCRIP
			ORDER BY $orno $ascdesc"; 
		}
		/** CONSULTA PARA VER DETALLES 
		*/
	...
	$queryEDetalle = "SELECT DISTINCT 
		...
		WHERE
		  ...
		  $whereTipoRadicado 
                  ";
		$orderE = "	ORDER BY $orno $ascdesc";
                $condicionUS = " and r.radi_depe_actu = $depeUs  "; 
                        /** CONSULTA PARA VER TODOS LOS DETALLES 
                         */ 
                       $queryETodosDetalle = $queryEDetalle . $orderE;
