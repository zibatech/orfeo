...
switch($db->driver)
{
	...
	{	
			...
			$queryE = "	SELECT
						...
						WHERE ".$db->conn->SQLDate('Y/m/d', 'r.radi_fech_radi')." BETWEEN '$fecha_ini' AND '$fecha_fin'  AND r.SGD_TRAD_CODIGO = 1
							 $whereActivos $whereTipoRadicado 
							GROUP BY usuario.USUA_NOMB ORDER BY $orno $ascdesc";
		...
		$filtroUsuario = " AND htev.depe_codi=$depeUsu and htev.usua_codi=$codUs";
		...
		$queryEDetalle = "SELECT
                            ...
                            dep.DEPE_NOMB,
                            renv.SGD_RENV_FECH, 
                            renv.SGD_RENV_NOMBRE, 
                            renv.SGD_RENV_DIR, 
                            fenv.SGD_FENV_DESCRIP,
                            renv.SGD_RENV_PLANILLA,  
                            renv.SGD_RENV_OBSERVA,
                            uu.USUA_LOGIN
						FROM
                            ...
                            LEFT JOIN sgd_renv_regenvio renv ON r.radi_nume_radi = renv.radi_nume_sal left join dependencia dep on (renv.depe_codi=dep.depe_codi) left join usuario uu on (renv.usua_doc=cast (uu.usua_doc as numeric)) left join sgd_fenv_frmenvio fenv ON renv.sgd_fenv_codigo = fenv.sgd_fenv_codigo
						WHERE
							r.radi_depe_actu=da.depe_codi
							AND r.RADI_DEPE_RADI=df.DEPE_CODI
                            
							$filtroUsuario
							AND TO_CHAR(r.radi_fech_radi,'YYYY/MM/DD') BETWEEN '$fecha_ini' AND '$fecha_fin'  AND r.SGD_TRAD_CODIGO = 1 $whereTipoRadicado $whereDependencia  ";
...
if($genDetalle==1){
	$titulos=array("#","1#RADICADO","2#FECHA RADICADO","3#TIPO DOCUMENTO","4#ASUNTO","5#USUARIO","6#RADICADO SALIDA","7#DEPENDENCIA", "8#FECHA R. SALIDA",  "9#DESTINATARIO" ,  "10#DIRECCION", "11#TIPO ENVIO","12#PLANILLA","13#OBSERVACIONES","14#REALIZO ENVIO");
}
else 		
	$titulos=array("#","1#Usuario","2#Radicados con datos de envio","2#VER");
...
function pintarEstadistica($fila,$indice,$numColumna)
{
	...
	case "1#Usuario":	
		$salida=$fila['USUARIO'];
		break;
	case "2#Radicados con datos de envio":
...
function pintarEstadisticaDetalle($fila,$indice,$numColumna)
{
	...
	case "5#USUARIO":
		...
		break;	
	case "6#RADICADO SALIDA":
		$salida="<center class=\"leidos\">".$fila['RADI_NUME_SAL']."</center>";			
        break;
    case "7#DEPENDENCIA":
        $salida="<center class=\"leidos\">".$fila['DEPE_NOMB']."</center>";			
        break;
    case "8#FECHA R. SALIDA":
        $salida="<center class=\"leidos\">".$fila['SGD_RENV_FECH']."</center>";			
        break;
    case "9#DESTINATARIO":
        $salida="<center class=\"leidos\">".$fila['SGD_RENV_NOMBRE']."</center>";			
        break;
    case "10#DIRECCION":
        $salida="<center class=\"leidos\">".$fila['SGD_RENV_DIR']."</center>";			
        break;
    case "11#TIPO ENVIO":
        $salida="<center class=\"leidos\">".$fila['SGD_FENV_DESCRIP']."</center>";			
        break;
    case "12#PLANILLA":
        $salida="<center class=\"leidos\">".$fila['SGD_RENV_PLANILLA']."</center>";			
        break;
    case "13#OBSERVACIONES":
        $salida="<center class=\"leidos\">".$fila['SGD_RENV_OBSERVA']."</center>";			
        break;
    case "14#REALIZO ENVIO":
        $salida="<center class=\"leidos\">".$fila['USUA_LOGIN']."</center>";			
        break;
