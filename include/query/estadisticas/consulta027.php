...
switch($db->driver)
{
	...
	{	
		...
		$filtroUsuario = " htev.depe_codi=$depeUsu and htev.usua_codi=$codUs AND";
		...
		$queryEDetalle = "	SELECT 
							...
							INNER JOIN (select h1.radi_nume_radi as radi_nume_radi, h1.id, h1.sgd_ttr_codigo as sgd_ttr_codigo, h1.usua_codi as usua_codi, h1.usua_doc as usua_doc, h1.depe_codi as depe_codi from hist_eventos h1 INNER JOIN (select distinct hist_eventos.radi_nume_radi as radi_nume_radi, min(hist_eventos.id) as id from hist_eventos where hist_eventos.sgd_ttr_codigo = 2 GROUP BY hist_eventos.radi_nume_radi) h2 on h1.id = h2.id ) htev ON r.radi_nume_radi = htev.radi_nume_radi 
							...
							LEFT JOIN anexos anx ON r.radi_nume_radi=anx.anex_radi_nume 
							LEFT JOIN hist_eventos hst ON r.radi_nume_radi=hst.radi_nume_radi  and hst.sgd_ttr_codigo=42
							WHERE 
							$filtroUsuario
							r.radi_depe_actu=da.depe_codi AND 
							r.RADI_DEPE_RADI=df.DEPE_CODI AND 
							TO_CHAR(r.radi_fech_radi,'YYYY/MM/DD') BETWEEN '$fecha_ini' AND '$fecha_fin' $whereTipoRadicado $whereDependencia ";
...
if($genDetalle==1){
	$titulos=array("#","1#RADICADO","2#FECHA RADICADO","3#TIPO DOCUMENTO","4#ASUNTO","5#USUARIO","6#IMAGEN","7#FECHA_IMAGEN","8#ARCHIVO_ANEXO","9#FECHA_ANEXO","10#CREADOR");
}
else 		
	$titulos=array("#","1#USUARIO","2#Radicados Sin Imagen","1#VER");
...
function pintarEstadisticaDetalle($fila,$indice,$numColumna)
{
	...
	case "5#USUARIO":
		...
		break;	
	case "6#IMAGEN":
		$salida="<center class=\"leidos\">".$fila['IMAGEN']."</center>";			
		break;
	case "7#FECHA_IMAGEN":
		$salida="<center class=\"leidos\">".$fila['FECHA_IMAGEN']."</center>";			
		break;
