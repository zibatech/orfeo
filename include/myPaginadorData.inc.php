...
class myPaginador{
  ...
  function __construct(&$db,$sql,$ordenadoPor,$tipodOrden="ASC",$tamPagina=2000,$id="adodb",$showPageLinks = true){
    $this->myPaginador($db,$sql,$ordenadoPor,$tipodOrden="ASC",$tamPagina=2000,$id="adodb",$showPageLinks = true);
  }
...
;
	...
	*/
	...
	function generarPagina($columnasTitulo,$claseTitulo="titulos3",$noHayResultados='NO se Encontraron Resultados'){
		...
		<form action=\"{$this->url}\" method=\"GET\" name=\"{$this->id}formaPaginador\" id=\"{$this->id}formaPaginador\" style=\"padding-top: 70px;\" >
		\n
		<table class=\"table table-bordered table-striped display compact\" id=\"muchasid\"  ";
		foreach ($this->propiedadesTabla as $clave=>$valor){
			echo" ".$clave."=\"".$valor."\" ";
		}
		echo " >";
		$claseTitulo=($claseTitulo)?"class=\"$claseTitulo\"":"";
		echo "\t \n  <thead><tr {$claseTitulo}> \n ";
		...
		if ($i===$registroInicio){
		  echo "\t <tr> \n \t \t <td colspan=\"{$numCols}\" class=\"alarmas\"  align=\"center\"> \n \t \t {$noHayResultados} \n </td> \n \t </tr>	";
			...
			echo "\t \n  <tfoot><tr style=\"background-color: #46AE4C; color: #fff; font-size: 12px;\"> \n ";
			foreach($columnasTitulo as $clave=>$col){
				echo "\t  <th>  \n ";
				$posalm=strpos($col,"#");
				if(!$posalm){
