...
if($contrato)
{
	$sugerido = Informe::last($contrato->id);

	if (!$sugerido)
	{
		$sugerido = new Informe;
		$sugerido->numero = 0;
		$sugerido->fecha_inicio = '';
		$sugerido->fecha_fin = Carbon\Carbon::now()->format('Y-m-d');
		$sugerido->valor_ejecutado = $contrato->honorarios_mensuales;
		$sugerido->total_ejecutado = 0;
	}
...
<body>
	<form action="modal_informes_save.php" method="POST" enctype="multipart/form-data">
		<div class="container-fluid">
			...
			<div class="row">
				<div class="col-sm-3 mt-3">
					...
					<input type="number" class="form-control" id="numero" name="numero" aria-describedby="Informe Nº" value="<?= $informe ? $informe->numero : $sugerido->numero + 1 ?>" required>
				</div>
			</div>
			<div class="row">
			...
			<div class="row">
				<div class="col mt-3" id="valor_ejecutado">
					...
					<input type="text" class="form-control" name="valor_ejecutado" value="<?= $informe ? $informe->valor_ejecutado : $sugerido->valor_ejecutado ?>">
					<small class="en_letras"></small>
				</div>
				<div class="col mt-3" id="total_ejecutado">
					<label>Valor ejecutado</label>
					<input type="text" class="form-control" readonly name="total_ejecutado" data-initial-value="<?= $informe ? $informe->total_ejecutado - $informe->valor_ejecutado : $sugerido->total_ejecutado ?>" value="<?= $informe ? $informe->total_ejecutado : $sugerido->total_ejecutado ?>">
					<small class="en_letras"></small>
				</div>
			</div>
			<div class="row">
			...
			<div class="row">
				<div class="col mt-3">
					...
					<input type="date" class="form-control" id="fecha_inicio" name="fecha_inicio" aria-describedby="Fecha inicio" value="<?= $informe ? $informe->fecha_inicio : '' ?>" required>
				</div>
				<div class="col mt-3">
					<label for="fecha_fin" class="form-label">Fecha fin</label>
					<input type="date" class="form-control" id="fecha_fin" name="fecha_fin" aria-describedby="Fecha fin" value="<?= $informe ? $informe->fecha_fin : '' ?>" required>
				</div>
			</div>
			<div class="row">
				<div class="col">
			...
			<?php foreach ($obligaciones as $key => $obligacion): ?>
				...
				<div class="row">
					<div class="col mt-5">
						...
						<textarea id="obligacion_descripcion_<?= $obligacion->id ?>" class="form-control" name="obligacion_descripcion[<?= $obligacion->id ?>]" required><?= $obligacion_informe ? $obligacion_informe->descripcion : '' ?></textarea>
					</div>
				</div>
				<div class="row">
					<div class="col mt-3" style="display: none;">
						<label id="obligacion_fecha_inicio_<?= $obligacion->id ?>">Desde</label>
						<input type="date" id="obligacion_fecha_inicio_<?= $obligacion->id ?>" class="form-control" name="obligacion_fecha_inicio[<?= $obligacion->id ?>]" value="<?= $obligacion_informe ? $obligacion_informe->fecha_inicio : '' ?>">
					</div>

					<div class="col mt-3" style="display: none;">
						<label id="obligacion_fecha_fin_<?= $obligacion->id ?>">Hasta</label>
						<input type="date" id="obligacion_fecha_fin_<?= $obligacion->id ?>" class="form-control" name="obligacion_fecha_fin[<?= $obligacion->id ?>]" value="<?= $obligacion_informe ? $obligacion_informe->fecha_inicio : '' ?>">
					</div>

					<div class="col mt-3">
						<label id="obligacion_evidencias_<?= $obligacion->id ?>">Evidencias</label>
						<input type="file" multiple id="obligacion_evidencias_<?= $obligacion->id ?>" class="form-control" name="obligacion_evidencias[<?= $obligacion->id ?>][]">
					</div>
				</div>
								...
								<table class="table table-sm">
									...
									<tbody>
										<?php foreach ($evidencias as $key => $evidencia): ?>
											<tr>
												<td>
													<a href="#" data-evidencia="<?= $evidencia->nombre ?>" data-inf="<?= $informe->id ?>" data-id="<?= $evidencia->id ?>" class="eliminar btn btn-link btn-sm">
	...
	<script type="text/javascript">
		$(function(e) {
			...
			$('a.eliminar').on('click', function(e) {
		    	var ruta = "<?= $ruta_raiz.'/contratistas/modal_informes_evidencias_delete.php?inf=' ?>"
				var informe_id = $(this).data('inf');
				var id = $(this).data('id');
				var evidencia = $(this).data('evidencia');
				var resultado = confirm("Esta seguro que desea eliminar la evidencia: "+evidencia);
				if (resultado) 
				{
					window.location.href = ruta+informe_id+'&evi='+id;
				}
