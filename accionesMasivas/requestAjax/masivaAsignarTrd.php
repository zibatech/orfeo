...
include_once('dataCommon.php');
  ...
  if(!empty($radConTrd) && $cambExiTrd == 111){
    ...
    $sqlhis="	SELECT
		...
		FROM
		  SGD_RDF_RETDOCF      SG
		  JOIN SGD_MRD_MATRIRD MR ON (MR.SGD_MRD_CODIGO  = SG.SGD_MRD_CODIGO)
		  JOIN SGD_SBRD_SUBSERIERD  SU ON (MR.SGD_SBRD_ID = SU.SGD_SBRD_ID AND MR.SGD_SRD_ID  = SU.SGD_SRD_ID)
		  JOIN SGD_SRD_SERIESRD     SE ON (MR.SGD_SRD_ID  = SE.SGD_SRD_ID)
		  JOIN SGD_TPR_TPDCUMENTO   TD ON (MR.SGD_TPR_CODIGO  = TD.SGD_TPR_CODIGO)
		WHERE
